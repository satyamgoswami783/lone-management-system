        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 print:p-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="glass p-8 rounded-[40px] border-slate-800/50 flex flex-col md:flex-row md:items-center justify-between gap-8 h-full bg-slate-900/40">
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-2xl font-bold text-slate-100">{application.status}</p>
                                    <p className="text-sm text-slate-500">Submitted on {new Date(application.date).toLocaleDateString()}</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <DataSection
                            ...
                            items={[
                                ...
                                { label: 'National ID / Passport', value: application.idNumber || 'N/A' },
                                { label: 'Email Correspondence', value: application.email },
                                { label: 'Phone Number', value: '+27 (0) 82 123 4567' },
                            ]}
                        />
                        {/* Employment Info */}
                        <DataSection
                            ...
                            icon={Building2}
                            items={[
                                { label: 'Registered Employer', value: application.company },
                                { label: 'Monthly Gross Income', value: `R ${application.salary?.toLocaleString()}` },
                                { label: 'Work Designation', value: 'Senior Associate' },
                                { label: 'Payment Method', value: 'Payroll Deduction' },
                            ]}
                        />
                    </div>
                    ...
                    <div className="glass p-6 lg:p-10 rounded-[32px] lg:rounded-[40px] border-slate-800/50 space-y-6 lg:space-y-8 bg-gradient-to-br from-slate-900 to-slate-950">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 lg:gap-8">
                            ...
                            <MetricBox label="Total Repayable" value={`R ${(application.amount + 600).toLocaleString()}`} color="text-emerald-400 font-black" />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-8">
                    {/* Loan Timeline */}
                    <div className="glass p-8 rounded-[40px] border-slate-800/50 space-y-8 h-fit bg-slate-900/60 h-full">
                            ...
                            {application.auditHistory?.map((log, i) => (
                                <div key={i} className="relative pl-10">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-sm font-bold text-slate-200">{log.status}</p>
